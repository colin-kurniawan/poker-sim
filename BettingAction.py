...
class BettingAction:
    def __init__(self, board, players, under_the_gun_index, small_blind_index, big_blind_index, pot): 
        self.board = board
        ...
        self.pot_manager = PotManager(self.main_pot, self.side_pots)
        ...
        while True: 
            try: 
                ...
                if bet_size < player_range_chips and bet_size > 0: 
                    self.players[current_player_index].increment_chips(bet_size * -1)
                    self.main_pot += bet_size
    ...
    def call(self, current_player_index): 
        self.players[current_player_index].increment_chips(self.current_bet * -1)
    ...
    def all_in(self, current_player_index): 
        all_in_size = self.players[current_player_index].get_chips()
        self.players[current_player_index].increment_chips(all_in_size * -1)
        self.players[current_player_index].set_all_in()

        if all_in_size >= self.current_bet: 
            self.main_pot += all_in_size
            self.current_bet = all_in_size
        else: 
            self.pot_manager.add_side_pot()
            self.main_pot = self.pot_manager.update_main_pot()
            self.side_pots = self.pot_manager.update_side_pot()
        ...
        if self.current_bet == 0: 
            while True: 
                try:
                    ...
                    if action != "bet" and action != "all-in" and action != "fold":
                        raise ValueError
                    return action 
                except ValueError: 
                    print("Invalid Option")
